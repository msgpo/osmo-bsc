					...
					const char *endpoint_str_fmt, ...)
{
	...
	rc = snprintf(mgwep->endpoint, sizeof(mgwep->endpoint), endpoint_str_fmt, ap);
	va_end(ap);

	if (rc <= 0 || rc >= sizeof(mgwep->endpoint)) {
		LOG_MGWEP(mgwep, LOGL_ERROR, "Endpoint name too long, truncated: %s",
			  mgwep->endpoint);
			     ...
			     void *notify_data)
{
	...
	*ci = cleared_ci;
	...
	LOG_CI(ci, LOGL_ERROR, "%s, notify=%p %s", mgcp_verb_name(ci->verb), ci->notify, ci->notify ?
	       ci->notify->id : "");
...
mgwep_st_unused#MGWEP_ST_UNUSED
mgwep_st_wait_mgw_response#MGWEP_ST_WAIT_MGW_RESPONSE
mgwep_st_in_use#MGWEP_ST_IN_USE


:s/\(.*\)_st_\(.*\)#\(.*_ST_\(.*\)\)/static void \1_fsm_\2_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)\r{\r\tGET_MGWEP();\r}\r\rstatic void \1_fsm_\2(struct osmo_fsm_inst *fi, uint32_t event, void *data)\r{\r\tGET_MGWEP();\r\tswitch (event) {\r\tdefault:\r\t\tOSMO_ASSERT(false);\r\t}\r}\r




:s/\(.*\)_st_\(.*\)#\(MGWEP_ST_\(.*\)\)/\t[\3] = {\r\t\t.name = "\4",\r\t\t.onenter = \1_fsm_\2_onenter,\r\t\t.action = \1_fsm_\2,\r\t\t.in_event_mask = 0\r\t\t\t| S()\r\t\t\t,\r\t\t.out_state_mask = 0\r\t\t\t| S()\r\t\t\t,\r\t},
...
static void mgwep_fsm_check_state_chg_after_response(struct osmo_fsm_inst *fi)
{
	...
	if (!occupied)  {
		mgwep_fsm_state_chg(MGWEP_ST_UNUSED);
		return;
	}
