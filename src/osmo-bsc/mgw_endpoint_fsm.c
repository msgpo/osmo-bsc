			     ...
			     void *notify_data)
{
	...
	*ci = cleared_ci;
	...
	LOG_CI_VERB(ci, LOGL_DEBUG, "notify=%s", osmo_fsm_inst_name(ci->notify));
...
static int send_verb(struct mgwep_ci *ci)
{
	...
	case MGCP_VERB_MDCX:
		...
		ci->sent = true;
		if (rc) {
			LOG_CI_VERB(ci, LOGL_ERROR, "Cannot send (rc=%d %s)", rc, strerror(-rc));
			on_failure(ci);
		}
		break;
			...
			int *waiting_for_response)
{
	...
	for (i = 0; i < ARRAY_SIZE(mgwep->ci); i++) {
		...
		if (ci->pending)
			LOG_CI_VERB(ci, LOGL_DEBUG, "%s", ci->sent ? "waiting for response" : "waiting to be sent");
		else
			LOG_CI_VERB(ci, LOGL_DEBUG, "%s", mgcp_conn_peer_name(mgwep_ci_get_rtp_info(ci)));
