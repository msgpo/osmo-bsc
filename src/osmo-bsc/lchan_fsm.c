...
lchan_st_wait_ts_ready#LCHAN_ST_WAIT_TS_READY
lchan_st_wait_activ_ack#LCHAN_ST_WAIT_ACTIV_ACK
lchan_st_wait_rll_establish#LCHAN_ST_WAIT_RLL_ESTABLISH
lchan_st_wait_mgw_endpoint_available#LCHAN_ST_WAIT_MGW_ENDPOINT_AVAILABLE
lchan_st_wait_ipacc_crcx_ack#LCHAN_ST_WAIT_IPACC_CRCX_ACK
lchan_st_wait_ipacc_mdcx_ack#LCHAN_ST_WAIT_IPACC_MDCX_ACK
lchan_st_active#LCHAN_ST_ACTIVE
lchan_st_wait_sapis_released#LCHAN_ST_WAIT_SAPIS_RELEASED
lchan_st_wait_before_rf_release#LCHAN_ST_WAIT_BEFORE_RF_RELEASE
lchan_st_wait_rf_release_ack#LCHAN_ST_WAIT_RF_RELEASE_ACK
lchan_st_wait_after_error#LCHAN_ST_WAIT_AFTER_ERROR
lchan_st_borken#LCHAN_ST_BORKEN


:s/lchan_st_\(.*\)#\(LCHAN_ST_\(.*\)\)/static void lchan_fsm_\1_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)\r{\r\tGET_LCHAN(fi, lchan);\r}\r\rstatic void lchan_fsm_\1(struct osmo_fsm_inst *fi, uint32_t event, void *data)\r{\r\tGET_LCHAN(fi, lchan);\r\tswitch (event) {\r\tdefault:\r\t\tOSMO_ASSERT(false);\r\t}\r}\r


:s/lchan_st_\(.*\)#\(LCHAN_ST_\(.*\)\)/\t[\2] = {\r\t\t.name = "\3",\r\t\t.onenter = lchan_fsm_\1_onenter,\r\t\t.action = lchan_fsm_\1,\r\t\t.in_event_mask = 0\r\t\t\t| S(LCHAN_EV_RELEASE)\r\t\t\t,\r\t\t.out_state_mask = 0\r\t\t\t| S(LCHAN_ST_UNUSED)\r\t\t\t,\r\t},
					 ...
					 const char *file, int line)
{
	...
	case FOR_VTY:
		LOG_LCHAN(lchan, LOGL_ERROR, "VTY user invoked lchan activation failed");
		break;

	default:
		LOG_LCHAN(lchan, LOGL_ERROR, "lchan activation failed");
		break;
	}
}
...
uint32_t lchan_fsm_on_error[32] = {
	...
	[LCHAN_ST_ACTIVE] 			= LCHAN_ST_WAIT_SAPIS_RELEASED,
...
static void lchan_fsm_wait_rll_establish_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	GET_LCHAN(fi, lchan);
	if (lchan->release_requested)
		lchan_fail_to(LCHAN_ST_WAIT_SAPIS_RELEASED, "Release requested while activating");
...
static void lchan_fsm_wait_rll_establish(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case LCHAN_EV_RLL_ESTABLISH_IND:
		...
		if (lchan->activate.requires_voice_stream) {
			...
			lchan_fsm_state_chg(LCHAN_ST_ACTIVE);
		break;
...
static void lchan_fsm_active_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	GET_LCHAN(fi, lchan);
...
static const struct osmo_fsm_state lchan_fsm_states[] = {
	...
	[LCHAN_ST_ACTIVE] = {
		.name = "ACTIVE",
		.onenter = lchan_fsm_active_onenter,
...
void lchan_fsm_cleanup(struct osmo_fsm_inst *fi, enum osmo_fsm_term_cause cause)
{
	GET_LCHAN(fi, lchan);
	if (lchan->conn)
		gscon_forgetx_lchan(lchan->conn, lchan);
	lchan_reset(lchan);
	if (lchan->last_error) {
...
void lchan_forgetx_conn(struct gsm_lchan *lchan)
{
	if (!lchan)
		return;
	lchan_forgetx_mgw_endpoint(lchan);
	lchan->conn = NULL;
}
