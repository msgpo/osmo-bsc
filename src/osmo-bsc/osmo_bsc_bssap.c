...
static bool sockaddr_to_assignment_req(struct assignment_request *req,
				       const struct sockaddr_storage *sa)
{
	...
	req->msc_rtp_port = osmo_ntohs(sin->sin_port);

	rc = osmo_strlcpy(req->msc_rtp_addr, inet_ntoa(sin->sin_addr), sizeof(req->msc_rtp_addr));
	if (rc <= 0 || rc >= sizeof(req->msc_rtp_addr))
		return false;
				     ...
				     struct msgb *msg, unsigned int length)
{
	...
	case GSM0808_CHAN_SPEECH:
		...
		if (aoip) {
			if (!sockaddr_to_assignment_req(&req, &rtp_addr)) {
			 ...
			 * (the MSC does that for us). */
			req.msc_rtp_port = mgcp_timeslot_to_port(multiplex, timeslot, msc->rtp_base);
		}
		break;
...
int bsc_send_handover_request_ack(struct gsm_subscriber_connection *conn, struct msgb *rr_ho_command)
{
	struct msgb *msg;
