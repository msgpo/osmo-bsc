 ...
 * 		{ .T=7, .default_val=60, .desc="Water Boiling Timeout" },  // default is .unit=T_S == 0
 ...
 *
 * 	val = T_def_get(global_T_defs, 7, T_S, -1); // -> 10
 * 	sleep(val);
 *
 * 	val = T_def_get(global_T_defs, 7, T_M, -1); // 10 seconds becomes 1 minute -> 1
 * 	sleep_minutes(val);
 *
