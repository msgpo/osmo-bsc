...
/*
assignment_st_requested#ASSIGNMENT_ST_REQUESTED
assignment_st_wait_lchan#ASSIGNMENT_ST_WAIT_LCHAN_ACTIVE
assignment_st_wait_mgw_endpoint_to_msc#ASSIGNMENT_ST_WAIT_MGW_ENDPOINT_TO_MSC
assignment_st_wait_ass_complete#ASSIGNMENT_ST_WAIT_RR_ASS_COMPLETE
assignment_st_success#ASSIGNMENT_ST_SUCCESS
assignment_st_failure#ASSIGNMENT_ST_FAILURE

:s/assignment_st_\(.*\)#\(ASSIGNMENT_ST_\(.*\)\)/static void assignment_fsm_\1_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)\r{\r\tGET_CONN();\r}\r\rstatic void assignment_fsm_\1(struct osmo_fsm_inst *fi, uint32_t event, void *data)\r{\r\tGET_CONN();\r\tswitch (event) {\r\tdefault:\r\t\tOSMO_ASSERT(false);\r\t}\r}\r



:s/assignment_st_\(.*\)#\(ASSIGNMENT_ST_\(.*\)\)/\t[\2] = {\r\t\t.name = "\3",\r\t\t.onenter = assignment_fsm_\1_onenter,\r\t\t.action = assignment_fsm_\1,\r\t\t.in_event_mask = 0\r\t\t\t| S(ASSIGNMENT_EV_CONN_RELEASING)\r\t\t\t,\r\t\t.out_state_mask = 0\r\t\t\t| S(ASSIGNMENT_ST_FAILURE)\r\t\t\t,\r\t},
...
static void assignment_success(struct gsm_subscriber_connection *conn)
{
	...
	/* Only remembered this for error handling */
...
static void assignment_fsm_wait_mgw_endpoint_to_msc(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case ASSIGNMENT_EV_MSC_MGW_OK:
		...
		if (!mgw_info) {
			assignment_fail(GSM0808_CAUSE_EQUIPMENT_FAILURE,
					"Unable to retrieve RTP port info allocated by MGW for the MSC side.");
			return;
		}
