	...
	if (ho->scope & (HO_INTRA_CELL | HO_INTRA_BSC)) {
		...
		if (ho->mt.new_lchan)
			snprintf(buf, sizeof(buf),
				 ...
				 ho->mt.inter_bsc.cell_id_serving_name,
...
static void handover_reset(struct gsm_subscriber_connection *conn)
{
	...
	ci = conn->ho.mt.created_ci_for_msc;
	if (ci) {
...
static void handover_start_intra_bsc(struct gsm_subscriber_connection *conn)
{
	...
	OSMO_ASSERT(ho->mt.new_bts);
	OSMO_ASSERT(ho->new_lchan_type != GSM_LCHAN_NONE);
	OSMO_ASSERT(!ho->mt.new_lchan);

	ho->scope = (ho->mt.new_bts == conn->lchan->ts->trx->bts) ? HO_INTRA_CELL : HO_INTRA_BSC;
	ho->mt.ho_ref = g_next_ho_ref++;
	ho->mt.async = true;

	ho->mt.new_lchan = lchan_select_by_type(ho->mt.new_bts, ho->new_lchan_type);
			     ...
			     struct handover_mt_req *req)
{
	...
	if (TLVP_PRESENT(tp, GSM0808_IE_AOIP_TRASP_ADDR)) {
		...
		if (!sockaddr_to_str_and_uint(req->msc_assigned_rtp_addr, sizeof(req->msc_assigned_rtp_addr),
					      &req->msc_assigned_rtp_port, &msc_rtp_sa)) {
			LOG_HO(conn, LOGL_ERROR, "Invalid MSC's RTP address (too long?)");
			return false;
		}
	} else if (aoip) {
		LOG_HO(conn, LOGL_ERROR,
				 ...
				 struct msgb *ho_request_msg)
{
	...
	struct handover_mt_req *req = &ho->mt.inter_bsc;
	int match_idx;
	...
	*ho = (struct handover){
	...
	ho_count(BSC_CTR_INTER_BSC_HO_MT_ATTEMPTED);
...
void handover_end(struct gsm_subscriber_connection *conn, enum handover_result result)
{
	...
	if (ho->scope & HO_INTER_BSC_MT) {
		if (result == HO_RESULT_OK) {
			if (!ho->mt.new_lchan) {
				LOG_HO(conn, LOGL_ERROR, "Inter-BSC HO MT ends in success,"
				       " but there is no lchan");
				result = HO_RESULT_ERROR;
			} else
				result = bsc_tx_bssmap_ho_complete(conn, ho->mt.new_lchan);
		}
	...
	if (result == HO_RESULT_OK)
		conn->ho.mt.created_ci_for_msc = NULL;
	...
	if ((ho->scope & HO_INTER_BSC_MT) && result == HO_RESULT_OK) {
		conn->user_plane.msc_assigned_cic = conn->ho.mt.inter_bsc.msc_assigned_cic;
		osmo_strlcpy(conn->user_plane.msc_assigned_rtp_addr,
			     conn->ho.mt.inter_bsc.msc_assigned_rtp_addr,
			     sizeof(conn->user_plane.msc_assigned_rtp_addr));
		conn->user_plane.msc_assigned_rtp_port = conn->ho.mt.inter_bsc.msc_assigned_rtp_port;
	}
...
static void ho_fsm_wait_mgw_endpoint_to_msc_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	if (!gscon_connect_mgw_to_msc(conn,
				      ho->mt.new_lchan,
				      ho->mt.inter_bsc.msc_assigned_rtp_addr,
				      ho->mt.inter_bsc.msc_assigned_rtp_port,
				      fi,
				      HO_EV_MSC_MGW_OK,
				      HO_EV_MSC_MGW_FAIL,
				      NULL,
					...
					const struct gsm0808_cell_id_list2 *target_cells)
{
	...
	ho_count(BSC_CTR_INTER_BSC_HO_MO_ATTEMPTED);
...
static const struct osmo_fsm_state ho_fsm_states[] = {
	...
	[MOHO_ST_WAIT_HO_COMMAND] = {
		.name = "inter-BSC-MO:WAIT_HO_COMMAND",
