			     ...
			     struct handover_mt_req *req)
{
	...
	bool aoip = (conn->sccp.msc->a.asp_proto != OSMO_SS7_ASP_PROT_IPA);
	...
	/* To be able to use gsm0808_cell_id_name() in logging without interfering with these cell id
	 * printouts, store the result. */
...
static void ho_fsm_wait_mgw_endpoint_to_msc_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	if (!gscon_connect_mgw_to_msc(conn,
				      ho->mt.new_lchan,
				      ho->mt.inter_bsc.msc_rtp_addr,
				      ho->mt.inter_bsc.msc_rtp_port,
				      fi,
				      HO_EV_MSC_MGW_OK,
				      HO_EV_MSC_MGW_FAIL,
				      NULL,
