...
static void ts_setup_lchans(struct gsm_bts_trx_ts *ts)
{
	...
	LOG_TS(ts, LOGL_DEBUG, "max lchans: %d", max_lchans);

	for (i = 0; i < max_lchans; i++) {
		/* We sometimes receive more than one Channel OPSTART ACK. Not quite sure why, just
		 * seeing it happen with bts_ipaccess_nanobts. Don't fail on the second init. */
		if (ts->lchan[i].fi)
			continue;
	...
	LOG_TS(ts, LOGL_DEBUG, "lchans initialized: %d", max_lchans);
}
...
static void ts_fsm_pdch_onenter(struct osmo_fsm_inst *fi, uint32_t prev_state)
{
	...
	if ((count = ts_lchans_waiting(ts))) {
		LOG_TS(ts, LOGL_DEBUG, "%d lchan(s) waiting for usable timeslot", count);
