...
static int rsl_rx_rf_chan_rel_ack(struct gsm_lchan *lchan)
{
	...
	/* Catch Osmocom style dyn TS PDCH release ACK: */
	if (ts->pchan_on_init == GSM_PCHAN_TCH_F_TCH_H_PDCH
	    && lchan->nr == 0
	    && ts->fi->state == TS_ST_WAIT_PDCH_DEACT) {
		osmo_fsm_inst_dispatch(ts->fi, TS_EV_PDCH_DEACT_ACK, NULL);
		return 0;
	}

	/* Otherwise, it is a regular lchan release. */
	osmo_fsm_inst_dispatch(lchan->fi, LCHAN_EV_RSL_RF_CHAN_REL_ACK, NULL);
	return 0;
}
...
static int rsl_rx_chan_act_ack(struct msgb *msg)
{
	...
	/* Catch Osmocom style dyn TS PDCH Activ ACK: */
	if (ts->pchan_on_init == GSM_PCHAN_TCH_F_TCH_H_PDCH
	    && lchan->nr == 0
	    && (rslh->chan_nr & RSL_CHAN_OSMO_PDCH) == RSL_CHAN_OSMO_PDCH) {
		osmo_fsm_inst_dispatch(ts->fi, TS_EV_PDCH_ACT_ACK, NULL);
		return 0;
	}

	/* Otherwise, it is a regular lchan act. */
	osmo_fsm_inst_dispatch(lchan->fi, LCHAN_EV_RSL_CHAN_ACTIV_ACK, NULL);
...
static int rsl_rx_chan_act_nack(struct msgb *msg)
{
	...
	if (!msg->lchan->fi) {
		LOGP(DRSL, LOGL_ERROR,
		     "%s CHANNEL ACTIVATE NACK on uninitialized lchan (chan_nr=0x%x)\n",
		     gsm_lchan_name(lchan), dh->chan_nr);
	}
	...
	if (TLVP_PRESENT(&tp, RSL_IE_CAUSE)) {
		...
		LOG_LCHAN(lchan, LOGL_ERROR, "CHANNEL ACTIVATE NACK: %s (%s)",
			  rsl_err_name(*cause), osmo_hexdump(cause, TLVP_LEN(&tp, RSL_IE_CAUSE)));
		osmo_fsm_inst_dispatch(lchan->fi, LCHAN_EV_RSL_CHAN_ACTIV_NACK, (void*)cause);
	} else {
		LOG_LCHAN(lchan, LOGL_ERROR, "CHANNEL ACTIVATE NACK lacks a cause IE");
		osmo_fsm_inst_dispatch(lchan->fi, LCHAN_EV_RSL_CHAN_ACTIV_NACK, 0);
	}
	return 0;
}
...
static int rsl_rx_conn_fail(struct msgb *msg)
{
	...
	LOG_LCHAN(lchan, LOGL_NOTICE, "CONNECTION FAIL");
...
static int abis_rsl_rx_dchan(struct msgb *msg)
{
	...
	lchan_name = gsm_lchan_name(msg->lchan);
	DEBUGP(DRSL, "%s %s\n", gsm_lchan_name(msg->lchan), rsl_or_ipac_msg_name(rslh->c.msg_type));

	if (!msg->lchan->fi) {
		LOGP(DRSL, LOGL_ERROR,
		     "Rx RSL DCHAN: RSL message for unconfigured lchan: chan_nr=0x%x %s trx=%p ts=%p=%p"
		     " lchan=%p=%p\n",
		     rslh->chan_nr, gsm_lchan_name(msg->lchan), sign_link->trx, msg->lchan->ts,
		     msg->lchan->ts->fi, msg->lchan, msg->lchan->fi);
		return -EINVAL;
	}
...
int rsl_tx_imm_ass_rej(struct gsm_lchan *lchan)
{
	uint8_t wait_ind;
	...
	return rsl_send_imm_ass_rej(bts, lchan->rqd_ref, wait_ind);
}
...
static int abis_rsl_rx_rll(struct msgb *msg)
{
	...
	msg->lchan = lchan_lookup(sign_link->trx, rllh->chan_nr,
				  "Abis RSL rx RLL: ");
	lchan_name = gsm_lchan_name(msg->lchan);
	DEBUGP(DRLL, "%s SAPI=%u ", lchan_name, sapi);
...
static int abis_rsl_rx_ipacc_dlcx_ind(struct msgb *msg)
{
	...
	LOG_LCHAN(msg->lchan, LOGL_NOTICE, "Rx IPACC DLCX IND");
...
/*! RSL has received a DATA INDICATION with L3 from MS */
int gsm0408_rcvmsg(struct msgb *msg, uint8_t link_id)
{
	...
	lchan = msg->lchan;
	if (!lchan->fi) {
		LOGP(DRSL, LOGL_INFO, "%s: Got data for uninitialized lchan, discarding.\n",
		     gsm_lchan_name(lchan));
		return -1;
	}
