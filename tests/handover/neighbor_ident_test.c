...
static const struct neighbor_ident_key *k(uint16_t arfcn, enum bsic_kind kind, uint16_t bsic)
{
...
#define check_get(key, expect_rc) \
	do { \
		struct gsm0808_cell_id_list2 *rc; \
...
int main(void)
{
	...
	nil = NULL;
	check_add(k(1, BSIC_6BIT, 2), &cgi1, -ENOMEM);
	check_get(k(1, BSIC_6BIT, 2), false);
	check_del(k(1, BSIC_6BIT, 2), false);

	printf("\n--- adding entries, test that no two identical entries are added\n");
	nil = neighbor_ident_init(ctx);
	check_add(k(1, BSIC_6BIT, 2), &cgi1, 1);
	check_get(k(1, BSIC_6BIT, 2), true);
	check_add(k(1, BSIC_6BIT, 2), &cgi1, 1);
	check_add(k(1, BSIC_6BIT, 2), &cgi2, 2);
	check_add(k(1, BSIC_6BIT, 2), &cgi2, 2);
	check_del(k(1, BSIC_6BIT, 2), true);

	printf("\n--- Cannot mix cell identifier types for one entry\n");
	check_add(k(1, BSIC_6BIT, 2), &cgi1, 1);
	check_add(k(1, BSIC_6BIT, 2), &lac1, -EINVAL);
	check_del(k(1, BSIC_6BIT, 2), true);
	neighbor_ident_free(nil);

	printf("\n--- BSIC matching: 6bit and 9bit are different realms, and wildcard match is weaker\n");
	nil = neighbor_ident_init(ctx);
	check_add(k(1, BSIC_NONE, 0), &cgi1, 1);
	check_add(k(1, BSIC_6BIT, 2), &lac1, 1);
	check_add(k(1, BSIC_9BIT, 2), &lac2, 2);
	check_get(k(1, BSIC_6BIT, 2), true);
	check_get(k(1, BSIC_9BIT, 2), true);
	printf("--- wildcard matches both 6bit and 9bit BSIC regardless:\n");
	check_get(k(1, BSIC_6BIT, 23), true);
	check_get(k(1, BSIC_9BIT, 23), true);
	neighbor_ident_free(nil);

	printf("\n--- Value ranges\n");
	nil = neighbor_ident_init(ctx);
	check_add(k(6, BSIC_6BIT, 1 << 6), &lac1, -ERANGE);
	check_add(k(9, BSIC_9BIT, 1 << 9), &lac1, -ERANGE);
	check_add(k(6, BSIC_6BIT, -1), &lac1, -ERANGE);
	check_add(k(9, BSIC_9BIT, -1), &lac1, -ERANGE);
	check_add(k(0, BSIC_NONE, 0), &cgi1, 1);
	check_add(k(65535, BSIC_NONE, 65535), &lac1, 1);
	check_add(k(0, BSIC_6BIT, 0), &cgi2, 2);
	check_add(k(65535, BSIC_6BIT, 0x3f), &lac2, 2);
	check_add(k(0, BSIC_9BIT, 0), &cgi1, 1);
	check_add(k(65535, BSIC_9BIT, 0x1ff), &cgi2, 2);
	...
	printf("\n--- size limits\n");
	{
		...
		i = neighbor_ident_add(nil, k(1, BSIC_6BIT, 2), &a);
		printf("Added first cell identifier list (added %u) --> rc = %d\n", a.id_list_len, i);
		i = neighbor_ident_add(nil, k(1, BSIC_6BIT, 2), &b);
